...
public:
    ...
    static constexpr const char*  description = "{\"name\":\"BMP280\",\"conf\":{\"sdaPin\":-1,\"sclPin\":-1,\"address\":\"0x76\"}}\"";
    ...
    BMP280Sensor( JsonObject& sensorConf): CSensor(sensorConf){
        ...
        Serial.println(sensorConf["config"]["address"].as<String>());
        Serial.println("ok till here");
        Serial.println((int)strtol(sensorConf["config"]["address"].as<char*>(), NULL, 16));
        Serial.println("ok till here");

        _addr = (int)strtol(sensorConf["config"]["address"].as<char*>(), NULL, 16);
        Serial.println("ok till here");
        Wire.begin(32,33);
