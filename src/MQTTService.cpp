...
MQTTService::MQTTService(std::string host, std::string credentials, std::string format, std::string target) :
...
void MQTTService::publishValue(const char* message) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    if ((client.connected()) || (!client.connected()&& reconnect())) {
        Serial.print("[MQTTService] sending message:");
        Serial.println(message);
        client.publish(_target.c_str(), message);
      } else {
        /* code */
        Serial.println("[MQTTService] Client is not connected, cannot send message.");
      }
  } else {
