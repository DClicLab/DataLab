...
interface IIndex { name: string, index: number };


function Calendar(props: ICalendarProps) {
  const defaultSelected: DateRange = {
    from: new Date(new Date().setHours(0, 0, 0, 0)),
    to: new Date(new Date().setHours(24, 0, 0, 0) - 1)
  };
    ...
    fetch(ENDPOINT_ROOT + 'indexes')
      ...
      .then(indexesJ => {
        ...
        Promise.all(p).then(responses => {
          ...
          setStatus("All data fetched. Processing...")
          ...
          Promise.all(p1).then(res => {
            ...
            const [points, sensorInfos] = getMergedDP(ldatapoints, 250);
  ...
  useEffect(() => {
    if (props.fsinfo.files) {
      ...
      var start = new Date(new Date(datemin * 1000).setHours(0, 0, 0, 0));
      var end = new Date(new Date(datemax * 1000).setHours(24, 0, 0, 0) - 1);
  ...
  return (
    <>
      <Box flexDirection="column" display="flex" alignItems="center">
        ...
        <Button className={classes.button} variant="contained" disabled={status != "none"} onClick={() => displayGraph(range)} color="primary" startIcon={<TimelineIcon />} disableElevation>
          Generate graphs
</Button>{status != "none" ? status : ""}
      </Box>
      {dataToGraph ?
